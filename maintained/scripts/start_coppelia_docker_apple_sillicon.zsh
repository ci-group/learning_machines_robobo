...
(cd "$tmpDir" && docker build --platform linux/amd64 --tag coppelia_sim .)

# Removing as safely as possible
rm "$tmpDir/to_open.ttt" "$tmpDir/CoppeliaSim.tar.xz" "$tmpDir/Dockerfile"
rmdir "$tmpDir"

docker run -it --rm --platform linux/amd64 -p "${2:-23000}:${2:-23000}" --name "coppelia_$apiPort" coppelia_sim "-GzmqRemoteApi.rpcPort=${2:-23000}"
